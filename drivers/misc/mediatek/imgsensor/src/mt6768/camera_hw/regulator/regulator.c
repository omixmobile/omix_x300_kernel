...
/*prize add by zhuzhengjiang for sham dual camera  20210630 start*/
...
void set_avdd_regulator(int status)
{
	...
	if(g_avdd_pregulator == NULL ) {
		pr_err("g_avdd_pregulator is NULL \n");
		return;
	}
	if(status ==1)
		{
...
{
	printk("set_dvdd_regulator  status=%d\n",status);
	if(g_dvdd_pregulator == NULL ) {
		pr_err("g_dvdd_pregulator is NULL \n");
		return;
	}
	if(status ==1)
		{
...
#endif
/*prize add by zhuzhengjiang for sham dual camera  20210630 end*/
...
static enum IMGSENSOR_RETURN regulator_init(void *pinstance)
{
	...
	/*prize add by zhuzhengjiang for sham dual camera  20210630 end*/
	#ifdef CONFIG_PRIZE_DUAL_CAMERA_ENABLE
		g_avdd_pregulator = regulator_get(pdevice, "cam2_vcama");
		g_dvdd_pregulator = regulator_get(pdevice, "cam3_vcamd");
	#endif
	/*prize add by zhuzhengjiang for sham dual camera  20210630 end*/
